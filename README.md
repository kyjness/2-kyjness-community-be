...
│   └── versions/                           # 마이그레이션(001 초기 ~ 006_chat_dm_tables 등)
...
| **채팅(DM)** | 1:1 방·메시지: **`GET /v1/chat/rooms/direct/{peer_user_id}`**, **`GET /v1/chat/rooms/{room_id}/messages`**(커서). 실시간은 WebSocket **`/v1/ws/chat`**(쿼리 **`token=`** Access JWT). Redis Pub/Sub로 다중 인스턴스 간 메시지 전달; DB 테이블은 **`006_chat_dm_tables`** 마이그레이션. |
