...
| **인증** | JWT (PyJWT), bcrypt는 **`asyncio.to_thread`**로 해시·검증. 비밀번호는 **`PASSWORD_PEPPER`**와 함께 저장. Redis: **RTR**(Refresh), Access **`jti` 블랙리스트**(로그아웃), Rate Limit, 동시 refresh·백그라운드 잡 **분산 락**. |
...
- **검색 레이어 최적화**: Elasticsearch(ES)를 전용 검색 엔진으로 도입하여 저장(PostgreSQL)과 검색의 책임을 분리. 한글 형태소 분석기(Nori)를 적용해 단순 키워드 매칭을 넘어선 초저지연(Sub-second) 전문 검색(Full-Text Search) 성능 확보.
- **하이브리드 검색 (Hybrid Search)**: Vector DB(pgvector)의 의미론적 유사도 검색과 Elasticsearch의 키워드 검색을 결합한 하이브리드 레이어 구축. '우리 아이 건강 척척박사(RAG)' 서비스와 연계하여 LLM에 전달되는 컨텍스트의 정밀도를 높이고 답변의 신뢰성 극대화.
- **검색 품질 고도화**: 사용자 활동량 증가에 따른 트래픽 급증에 대비하여 검색 엔진의 Scale-out 구조를 채택하고, 오타 교정(Fuzzy Search), 유의어 처리, 검색어 자동완성 등 고도화된 사용자 경험(UX) 제공.
