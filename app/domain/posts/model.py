    ...
    def update_post(
        ...
        *,
        ...
        if title is not None:
            db.execute(update(Post).where(Post.id == post_id).values(title=title, updated_at=datetime.now(timezone.utc)))
        if content is not None:
            db.execute(update(Post).where(Post.id == post_id).values(content=content, updated_at=datetime.now(timezone.utc)))
        if image_ids is not None:
            ...
            for img_id in to_delete:
                ...
                if not other:
                    db.execute(update(Image).where(Image.id == img_id, Image.deleted_at.is_(None)).values(deleted_at=datetime.now(timezone.utc)))
        return True

    @classmethod
    def delete_post(cls, post_id: int, db: Session) -> bool:
        from app.comments.model import Comment
        db.execute(update(Comment).where(Comment.post_id == post_id, Comment.deleted_at.is_(None)).values(deleted_at=datetime.now(timezone.utc)))
        ...
        for img_id in image_ids:
            ...
            if not other:
                db.execute(update(Image).where(Image.id == img_id, Image.deleted_at.is_(None)).values(deleted_at=datetime.now(timezone.utc)))
        r = db.execute(update(Post).where(Post.id == post_id, Post.deleted_at.is_(None)).values(deleted_at=datetime.now(timezone.utc)))
        return r.rowcount > 0
