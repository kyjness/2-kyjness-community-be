...
@router.get(
    "", status_code=200, response_model=ApiResponse[PaginatedResponse[PostResponse]]
)
async def get_posts(
    page: int = Query(1, ge=1, description="페이지 번호"),
    size: int = Query(10, ge=1, le=100, description="페이지 크기"),
    q: Optional[str] = Query(None, description="검색어 (title, content ILIKE)"),
    sort: Optional[str] = Query(None, description="정렬: latest|popular|views|oldest"),
    db: AsyncSession = Depends(get_slave_db),
):
    result, has_more, total = await PostService.get_posts(
        page=page, size=size, db=db, q=q, sort=sort
    )
    return ApiResponse(
...
async def record_view(
    ...
    await PostService.record_post_view(post_id, client_id, db=db)
