...
async def upload_image(image: UploadFile = File(..., description="이미지 파일"), image_type: Literal["profile", "post"] = Query("post", alias="type", description="profile/post"), user: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    return await controller.upload_image(file=image, user=user, purpose=image_type, db=db)
