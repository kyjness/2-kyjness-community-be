...
- [ ] **③ 실시간 견고화**
  ...
  - [x] 마감: 코드 리뷰(③ 범위, 정확성 2건 반영 — ① WS 한도를 parse 앞으로 이동: 잘못된 프레임
        스팸도 한도에 포함, ② WS 거부를 RATE_LIMIT_REJECTIONS 메트릭에 집계). 보안 관점 신규
        취약점 0(공격 표면 축소: WS 무제한 쓰기·차단 우회 DM 차단). 정리 2건은 #35로 이연.
        ※ 에이전트 세션 한도로 파인더/검증을 인라인 수행
