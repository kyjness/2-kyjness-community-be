...
> **초점**: 확장 가능·결함 내성(Fault-tolerant, 고장 나도 서비스가 버티도록 설계) 인프라, 장애 복구 및 고가용성(HA, 서비스를 가능한 한 끊기지 않게 유지)  
> **기준**: 백엔드/프론트엔드 실제 코드·설정을 반영한 **현재 구현(Current)** 과 **목표 아키텍처(Target)** 를 구분해 기술함. 인프라 단일 공급원은 [PuppyTalk Infra](https://github.com/kyjness/2-kyjness-community-infra) Terraform이며, 본 문서의 **Current**는 **`ecs.tf`(ECS 서비스·태스크/컨테이너, 초기 운영 비용을 고려한 가성비 운영)** · **`ec2_db.tf`(EC2 단일 인스턴스에 PostgreSQL·Redis 직접 설치)** 와 정합되게 서술한다. **Target** 컴퓨팅은 **`eks.tf` 기반 EKS(쿠버네티스)** 이며, **배포 파이프라인(EKS 대상) 검증은 완료**되었으나 **실제 프로덕션 컴퓨팅을 EKS로 전환·운용하는 것은 향후 적용 예정**이다. DB **Target**은 **RDS(Multi-AZ, 가용 영역 분산: 서울 내에서도 물리적으로 멀리 떨어진 두 데이터센터에 서버를 나누어 배치해 정전 등에 대비하는 방식)** 이다.  
...
| **DB**         | PostgreSQL (psycopg 비동기)                             | **Current**: Terraform `ec2_db.tf`로 EC2 **단일 인스턴스**에 **Self-hosted** PostgreSQL. **Target**: 트래픽·HA 요구 시 **AWS RDS (Multi-AZ, 가용 영역 분산: 서울 내에서도 물리적으로 멀리 떨어진 두 데이터센터에 서버를 나누어 배치해 정전 등에 대비하는 방식)** 로 이전 검토. 앱 스택(`engine.py`, Alembic)은 동일.              |
...
graph TD
    ...
    subgraph ECS_Plane["ECS — Terraform ecs.tf (Current)"]
        ...
        P1[WAS Task/Container<br/>가성비 운영]
        P2[WAS Task/Container<br/>가성비 운영]
        SVC --> P1
        SVC --> P2
    end
...
REST 트래픽과 별도로, **DM 채팅**은 브라우저가 `wss://`(TLS)로 **WebSocket**을 업그레이드해 **장시간 연결**을 유지한다. WAS 프로세스(Gunicorn **Worker**)마다 메모리가 분리되므로, **한 사용자의 소켓이 붙은 인스턴스**와 **메시지를 저장·발행하는 요청이 처리된 인스턴스**가 다를 수 있다. 이 간극을 메우기 위해 **Redis Pub/Sub**으로 채널(`puppytalk:channel:chat:dm` 등)에 **Fan-out(메시지 뿌려주기: 하나의 메시지를 접속한 모든 서버에 동시에 전달하는 방식)** 하여, 모든 워커가 메시지를 구독한 뒤 **로컬 `ConnectionManager`**만 해당 사용자 소켓으로 push한다. 이로써 **Sticky Session(같은 사용자를 항상 같은 서버로 붙이는 방식) 없이도 Scale-out**이 가능한 **Stateless WAS(서버가 사용자 상태를 들고 있지 않고 바깥 Redis·DB에 맡기는 패턴)** 패턴을 유지한다([architecture.md](architecture.md) §14 참고).
...
WebSocket은 **연결 자체**는 장시간 열려 있으나, 애플리케이션은 **메시지 처리 시에만** `AsyncSessionLocal()` 등으로 세션을 열고 `ChatService`에서 **짧은 트랜잭션(DB에 묶여 있는 시간을 짧게 유지해 다른 요청이 DB를 쓸 수 있게 하는 것)** 만 수행한 뒤 즉시 반환한다. **소켓 1개가 DB 커넥션 1개를 점유하지 않도록** 설계하여, WebSocket 동시 접속 폭주가 곧바로 **Connection Pool 고갈(접속 대기줄 꽉 참: DB가 동시에 받을 수 있는 한계치를 넘어서는 현상)** 로 이어지지 않도록 한다(인프라는 여전히 **풀 크기 × Worker 수 × 태스크(또는 Pod) 수 ≤ PostgreSQL `max_connections`** 로 상한을 맞춰야 하며, **Current**는 EC2 단일 노드의 DB 파라미터·인스턴스 크기가 상한이다).
...
| **Target** | **RDS PostgreSQL (Multi-AZ, 가용 영역 분산: 서울 내에서도 물리적으로 멀리 떨어진 두 데이터센터에 서버를 나누어 배치해 정전 등에 대비하는 방식)** · **Read Replica** | 트래픽·**RTO(복구 시간: 서비스가 죽었을 때 다시 살려내는 데 걸리는 시간)** 요구 충족 시 **Managed DB**로 이전, 읽기 분산·자동 페일오버 | 현재 Terraform에 **미포함**(목표 아키텍처) |
...
### 3.1 Multi-AZ(가용 영역 분산: 서울 내에서도 물리적으로 멀리 떨어진 데이터센터에 부하·복제를 나누는 방식) 분산 배치 전략 (Current vs Target)
...
| **WAS (태스크/컨테이너)** | **ECS 서비스 오토스케일링** 등으로 **태스크** 수를 부하에 맞게 확장; 클러스터/용량 한계 시 **용량(인스턴스·Fargate)** 조정이 뒤따름 | **1차**: 태스크 증설 → **2차**: 기반 용량 증설 | **Target(EKS)**: **HPA(트래픽·CPU 등에 따라 Pod 개수를 자동으로 늘려 주는 쿠버네티스 기능)** 로 Pod 증설 → 노드 부족 시 **노드 그룹 스케일아웃**; 리소스 쿼터·`max_size` 상한 설계 |
...
**정리**: **Current**는 **컴퓨트(ECS)** 쪽은 수평 확장·AZ 분산이 가능하나, **데이터 계층은 단일 EC2**로 **가용성 목표가 애플리케이션보다 낮다**는 점을 전제로 운영한다. **Target**은 **EKS(쿠버네티스, 배포 파이프라인 검증 완료·프로덕션 전환은 향후)** + **RDS Multi-AZ + (선택) Read Replica + Redis 분리**로 그 격차를 줄인다.

### 3.2 Target — EKS 환경: ALB(Ingress)·HPA·Managed Node Group 오토스케일링

**전제**: 아래 표는 **목표(Target) 컴퓨팅**인 **EKS(쿠버네티스)** 기준이다. **EKS 대상 배포 파이프라인 검증은 완료**되었으나, **실제 프로덕션 워로드를 EKS에서 돌리는 것은 향후 적용 예정**이며, **Current** 런타임은 §3.1의 **ECS**를 따른다.
...
| ---- | ---------------------------- | ----------------------------- |
| **RPO(데이터 복구 시점: 최악의 경우 몇 분 전 데이터까지 잃어도 되는가의 기준)** | **스냅샷·논리 백업 주기**에 의존 — 간격을 짧게 가져가면 RPO를 줄일 수 있으나 **비용·운영 부하**와 트레이드오프 | 동기 스탠바이로 **대개 수 초~분 단위** 이하; 자동 백업+PITR |
| **RTO(복구 시간: 서비스가 죽었을 때 다시 살려내는 데 걸리는 시간)** | **단일 인스턴스 SPOF(단일 고장점)** — 장애 시 **새 EC2 프로비저닝·복원·DNS/엔드포인트 전환** 등 **수동·준수동** 단계로 **시간이 길어질 수 있음** | **자동 페일오버**로 **수 분대**를 목표로 설계 가능 |
| **운영 목표(참고)** | 문서 전체 상한으로 **RTO 1시간·RPO 5분**을 두되, **Current는 그보다 불리할 수 있음**을 인지하고 **백업 빈도·런북**으로 방어 | 도입 후 동일 표를 **RDS SLO**에 맞게 갱신 |


**정리**: **Current**는 결함을 숨기지 않고 **단일 DB EC2의 SPOF(단일 고장점)** 를 전제로, **스냅샷·백업으로 RPO를 방어**하고 **RTO는 RDS 전환으로 개선**하는 로드맵을 둔다. **Target** 달성 시 §3.3·본 표의 우측 열이 운영 SLA의 기준이 된다.
...
| **EKS (Target)**            | **HPA** 가 **maxReplica**에 도달, Pending Pod 증가 | 노드 그룹 `max_size`·CA·인스턴스 타입 검토 (프로덕션 EKS 전환 후)             |
...
- **롤링·카나리·블루/그린**: **Current**는 **ECS 서비스 롤링/블루그린(태스크 정의 교체)** 등으로 새 이미지 반영. **Target(EKS)** 는 **`Deployment` 롤링** 또는 **별도 Service 가중치·카나리 CRD** 등; Jenkins/파이프라인에서 `kubectl`·Helm 배포 시 **이전 ReplicaSet 롤백** 전략을 문서화( EKS 프로덕션 전환 후 ).
...
| **DB 리스크 (Current)** | **단일 DB EC2 SPOF(단일 고장점)** — **EBS 스냅샷·논리 백업**으로 **RPO(데이터 복구 시점 기준)** 방어, **RTO(복구 시간)** 는 수동 복구에 민감. **Target**: **RDS Multi-AZ(가용 영역 분산) (+ Read Replica)** 로 페일오버·RTO 개선. |
| **컴퓨트 스케일 (Current)** | **ECS 서비스 오토스케일링** 등으로 태스크를 1차 확장, 용량·클러스터 한도 내에서 기반 용량 조정 (`ecs.tf` 등). **Target**: **HPA**가 Pod를 1차 확장, 자원 부족 시 **Managed Node Group(및 CA)** 이 노드를 확장 (`eks.tf` 상한 내, **파이프라인 검증 완료·프로덕션 전환은 향후**). |
...
| **DR**       | §3.5 — **Current vs Target** 별 **RTO(복구 시간)/RPO(데이터 복구 시점)** 인지 |
...
- **인프라 배포**: [PuppyTalk Infra](https://github.com/kyjness/2-kyjness-community-infra) Terraform이 단일 진실 공급원이다. **Current** 프로덕션 컴퓨팅은 **`ecs.tf`(ECS 서비스·태스크)** + **`ec2_db.tf`(PostgreSQL·Redis on EC2 단일 인스턴스)** 가 코드에 대응하며, 본 문서 §1~§3의 **Current** 서술은 이와 정합된다. **Target** 컴퓨팅은 **`eks.tf`(EKS)** 로 정의되어 있고 **배포 파이프라인(EKS 대상) 검증은 완료**되었으나, **실제 운영 워로드의 EKS 전환은 향후 적용 예정**이다. **RDS 등 DB Target**은 아직 동일 레포에 필수 리소스로 포함되지 않을 수 있다.
- **기능 확장(추후 도입 시)**  
...
위 목표들은 **Redis를 비용·Quota·Rate Limit의 중앙 허브**로 쓰고, **큐·별도 컴퓨트**로 무거운 연산을 빼는 방향과 맞닿는다. **현재**의 Redis **Critical Path(핵심 기능이 지나가는 경로)** 고가용성(**Multi-AZ**, 가용 영역 분산)·**짧은 DB 트랜잭션**·**Fan-out(메시지 뿌려주기)** 은 그 전 단계에서 이미 **운영 복잡도와 비용 리스크**를 줄이는 기반이 된다.
