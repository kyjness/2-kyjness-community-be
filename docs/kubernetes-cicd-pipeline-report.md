...
본 보고서는 PuppyTalk 백엔드의 **쿠버네티스 및 CI/CD 구축 결과**를 성과 중심으로 정리한 문서입니다. **제한된 예산·인력** 조건에서 실제로 구축·운영 중인 **Current(ECS 기반)** 과, **트래픽 급증을 가정한 Target(EKS·GitOps)** 을 동일 문서에서 대비했습니다. Target은 EKS·GitOps·무중단 전환 구성과 `k6` 부하 테스트까지 포함해 정리했습니다.
...
> - **실시간(DM·인앱 알림)** — **Target** 기준으로 Stateless **Pod** 와 Redis Pub/Sub를 맞춘 배포·프로브 설계를 서술했습니다(DM은 WebSocket, 알림은 SSE). **Current** 는 ECS **태스크** 단위이나, 앱 레벨(Redis Fan-out) 개념은 동일하게 적용했습니다.
>
...
### 4.3 인앱 알림(SSE)·Redis·PostgreSQL (요약)

DM(WebSocket) 외에 **인앱 알림**은 **`GET /v1/notifications/stream`** 으로 **SSE**(`text/event-stream`)를 구독하고, 알림 레코드는 **PostgreSQL**에 두며 실시간 전달은 **Redis Pub/Sub**(채팅과 채널 분리)로 처리합니다. **Ingress(ALB) 관점**에서는 WebSocket과 마찬가지로 **장시간 HTTP 연결**이므로 **Idle timeout**·연결 수·Redis 가용성을 **동일한 운영 포인트**로 보면 됩니다. Redis가 없으면 스트림은 거절(503)되고 REST 목록·읽음으로 동기화 가능합니다. 도메인·장애 반경·Current 스택과의 관계는 [infrastructure-reliability-design.md](infrastructure-reliability-design.md) **§1.2.3** 을 참고해 주십시오.
...
| **실시간** | Redis Pub/Sub + Stateless Pod(DM **WSS**, 인앱 알림 **SSE**); Redis가 Critical Path — 상세는 절 4.3·[infrastructure-reliability-design.md](infrastructure-reliability-design.md) §1.2.3 |
