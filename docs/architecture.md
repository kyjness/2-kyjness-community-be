...
| 파일 | 역할 |
|------|------|
| **router** | HTTP 엔드포인트 정의. `Depends(get_master_db, get_slave_db, get_current_user)` 등으로 DB·로그인 유저 주입. controller 호출 후 schema로 응답 매핑. |
| **controller** | 비즈니스 로직·권한 확인·트랜잭션 흐름. 복합 연산 시 `with db.begin():` 사용. DB 접근은 model에 위임. |
| **model** | DB 접근(CRUD·쿼리). Session만 사용. commit/rollback은 의존성 세션 스코프에서 처리. |
...
├── app/
│   ├── api/                           # API 라우트·의존성 진입
│   │   ├── dependencies/              # 통합 DI: 인증·DB세션·권한·쿼리파싱
│   │   │   ├── auth.py                # Bearer 검증, CurrentUser, TOKEN_EXPIRED/UNAUTHORIZED
│   │   │   ├── db.py                  # get_master_db, get_slave_db
│   │   │   ├── permissions.py         # require_post_author, require_comment_author
│   │   │   └── query.py               # parse_availability_query
│   │   └── v1.py                      # /v1 prefix 라우터 묶음
│   ├── common/                        # 공통 유틸·응답·로깅
│   │   ├── codes.py                   # API 응답 코드 상수
│   │   ├── response.py                # 공통 응답 포맷·raise_http_error
│   │   ├── validators.py              # 닉네임·비밀번호 형식 검증
│   │   └── logging_config.py          # 로깅 설정, RequestIdFilter
│   ├── core/                          # 설정·미들웨어·보안·스토리지
│   │   ├── config.py                  # 환경 변수 (ENV별 .env 로드)
│   │   ├── middleware/                # 프록시·요청ID·접근로그·속도제한·보안헤더
│   │   │   ├── proxy_headers.py       # Nginx 등에서 실제 클라이언트 IP 보정
│   │   │   ├── request_id.py          # X-Request-ID 생성, contextvars 설정
│   │   │   ├── access_log.py          # 4xx/5xx·소요시간 로그, DEBUG 시 X-Process-Time
│   │   │   ├── rate_limit.py          # Redis Fixed Window, OPTIONS 제외, Fail-open
│   │   │   └── security_headers.py    # CSP·X-Frame-Options 등
│   │   ├── security.py                # JWT Access/Refresh 생성·검증, 비밀번호 해시
│   │   ├── storage.py                 # 로컬/S3 파일 업로드
│   │   ├── exception_handlers.py      # 전역 예외 → { code, data [, message] } 통일
│   │   └── cleanup.py                 # asyncio 전용: 만료 세션·회원가입용 이미지 TTL 정리
│   ├── db/                             # DB 엔진·세션·연결
│   │   ├── base.py                    # SQLAlchemy Base, utc_now, Partial Update
│   │   ├── engine.py                  # Writer/Reader 엔진, READ ONLY(Reader)
│   │   ├── connection.py              # init_database, check_database, close_database
│   │   └── session.py                 # get_connection(비요청용), get_db(하위호환)
│   └── domain/                         # 기능별 도메인 (router → controller → model → schema)
│       ├── auth/                       # 로그인·로그아웃·리프레시·회원가입·/me
│       │   ├── controller.py          # JWT 발급·Redis 저장·revoke_refresh_for_user
│       │   ├── model.py                # 레거시 세션: revoke_sessions_for_user, cleanup_expired_sessions
│       │   ├── router.py               # login, logout, refresh, signup, GET /me
│       │   └── schema.py               # 인증 요청/응답 DTO
│       ├── users/                      # 프로필 조회·수정·비밀번호·탈퇴
│       ├── media/                      # 이미지 업로드 (회원가입용/로그인후), signupToken·ref_count
│       ├── posts/                      # 게시글 CRUD·피드·좋아요·조회수
│       └── comments/                   # 댓글 CRUD·목록 페이지네이션
│   └── main.py                         # 앱 진입점·lifespan·미들웨어·라우터·/health
├── alembic/                            # DB 마이그레이션 (WRITER_DB_URL 사용)
│   ├── env.py                          # 마이그레이션 환경·DB URL·메타데이터
│   ├── README                           # Alembic 사용법·stamp head 안내
│   ├── script.py.mako                  # 리비전 스크립트 템플릿
│   └── versions/                       # 리비전 파일 (수동 SQL 사용 시 비어 있을 수 있음)
├── docs/                               # 상세 문서·DDL·참고용 SQL
│   ├── architecture.md                 # 이 문서. 설계 이유·인증·DB·폴더·미들웨어·요청 흐름
│   ├── api-codes.md                    # API 응답 code와 HTTP 상태 코드 매핑
│   ├── infrastructure-reliability-design.md  # 인프라·신뢰성 설계
│   ├── puppytalkdb.sql                 # 참고용 DDL (수동 테이블 생성 시 사용)
│   ├── clear_db.sql                    # 데이터만 비우기용 스크립트
├── test/                               # pytest (auth, comments, health, media, posts, users 등)
...
└── .env.example                        # 환경 변수 예시 (.env.development 등으로 복사)
```
...
⑤ 의존성 (Depends, app/api/dependencies)
   → get_master_db / get_slave_db: 요청마다 Session 주입. 성공 시 commit, 예외 시 rollback, finally close.
...
⑦ Route 핸들러 → Controller → Model
   Model은 Session만 사용. commit/rollback은 의존성 세션 스코프에서 처리.
